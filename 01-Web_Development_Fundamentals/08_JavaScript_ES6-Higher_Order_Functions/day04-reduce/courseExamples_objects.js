...
const studentBest = estudantes.map((student) => {
  const subjects = (acc, currValue) => {
    if (currValue.nota > acc.nota) return currValue;
    else return acc;
  };

  return {
    name: student.nome,
    materia: student.materias.reduce(subjects).name
  }
});
